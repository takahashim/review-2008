#
# $Id: preprocessor.rb 3810 2008-01-05 17:17:39Z aamine $
#
...
module ReVIEW
  ...
  class Preprocessor
    ...
    def unindent(list, n)
      n = minimum_indent(list) unless n.kind_of?(Integer)
      re = /\A#{' ' * n}/
      list.map {|lineno, str| [lineno, str.sub(re,'')] }
    end

    def minimum_indent(list)
      list.map {|n, line|
        if line.strip.empty?
        then 100000   # is more than one line bytes
        else line.slice(/\A\s*/).size
        end
      }.min
    end
    ...
    def evaluate(path, chunk)
      ...
      chunk.map {|line|
        if /\# \$\d+/ =~ line.string
          ...
          line.map_string {|s|
            line.string.sub(/\$(\d+)/) { outputs[$1.to_i - 1] }
          }
        else
          line
        end
  ...
  class Repository
    ...
    def _parse_file(f)
      ...
      if curr.size > 1
        curr.delete 'WHOLE'
        curr.each do |k,v|
          $stderr.puts "#{filename()}: end not seen: #{k} (begin @#{v[0][0]})"
        end
